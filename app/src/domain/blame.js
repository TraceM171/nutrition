...
import { getNutrientVal, getIngredientNutrient } from './nutrients.js';
import { effectiveAmountG, getRecipeWeightG } from './recipes.js';

export function buildBlameNode(ing, nutrientKey, scaleFactor, recipes) {
  if (scaleFactor === undefined) scaleFactor = 1;
  if (ing.type === 'food') {
    const base = getNutrientVal(ing, nutrientKey);
    return { name: ing.name, isRecipe: false, value: base * scaleFactor, children: null };
  }
  if (ing.type === 'recipe') {
    ...
    const base = getIngredientNutrient(ing, nutrientKey, recipes);
    const children = r.ingredients
      .map(sub => buildBlameNode(sub, nutrientKey, fraction * scaleFactor, recipes))
...
export function getDayBlameTree(day, nutrientKey, plan, meals, recipes) {
  const mealNodes = [];
  meals.forEach(meal => {
    const ings = plan[day][meal] || [];
    if (!ings.length) return;
    const items = ings
      .map(ing => buildBlameNode(ing, nutrientKey, 1, recipes))
      .filter(n => n.value > 0.0005)
      .sort((a, b) => b.value - a.value);
...
export function getWeeklyBlameTree(nutrientKey, plan, days, meals, recipes) {
  const filledDays = days.filter(d => meals.some(m => (plan[d][m] || []).length > 0));
  const dayNodes = [];
  filledDays.forEach(day => {
    const mealNodes = getDayBlameTree(day, nutrientKey, plan, meals, recipes);
